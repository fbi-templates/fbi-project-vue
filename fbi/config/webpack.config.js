module.exports = (require, ctx) => {
  ...
  const DataForDefine = (function DataForDefinePlugin() {
    const data = Object.assign({}, webpackOpts.data.all || {}, webpackOpts.data[ctx.env])
  ...
  const config = {
    ...
    entry: {
      app: ctx.isProd
        ...
        : [nodeModulesPath + '/webpack-hot-middleware/client', path.join(root, 'src/index.js')]
    },
    output: {
    ...
    module: {
      rules: [
        ...
        },
        {
          ...
          query: {
            limit: 5000,
            name: ctx.isProd ? '/[name].[hash:8].[ext]' : 'img/[name].[ext]?[hash:8]'
          }
        },
        {
          test: /\.css$/,
    ...
    plugins: [
      ...
      ctx.isProd ? new ProgressBarPlugin() : new webpack.HotModuleReplacementPlugin(),
      ctx.isProd
        ...
        : noop,
      ctx.isProd ? new ExtractTextPlugin('app.[contenthash:8].css') : noop,
      ctx.isProd
